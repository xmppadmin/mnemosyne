...
class MnemoDB(object):
    ...
    def ensure_index(self):
        ...
        self.db.metadata.ensure_index([('ip', 1), ('date', 1)])
        ...
        for item in ndata:
            ...
            for collection, document in item.items():
                if collection is 'url':
                    ...
                    if 'ip' in document and 'date' in document:
                        query = {'ip': document['ip'], 'date': document['date']}
                        values = dict(document)
                        del values['ip']
                        del values['date']
                        self.db[collection].update(query, values, upsert=True)
                else:
